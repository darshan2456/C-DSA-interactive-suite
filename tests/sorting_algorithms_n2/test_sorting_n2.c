...
void test_cocktail_shaker_sort()
{
    /* empty array: the sort must touch nothing */
    int empty[1] = {42};
    cocktail_shaker_sort(empty, 0);
    assert(empty[0] == 42);

    /* single element */
    int single[1] = {7};
    cocktail_shaker_sort(single, 1);
    assert(single[0] == 7);

    /* already sorted ascending */
    int asc[5] = {1, 2, 3, 4, 5};
    cocktail_shaker_sort(asc, 5);
    assert(asc[0] == 1);
    assert(asc[1] == 2);
    assert(asc[2] == 3);
    assert(asc[3] == 4);
    ...
    /* random values (including negatives) */
    int random_vals[8] = {9, -3, 0, 7, -10, 5, 2, -3};
    cocktail_shaker_sort(random_vals, 8);
    assert(random_vals[0] == -10);
    assert(random_vals[1] == -3);
    assert(random_vals[2] == -3);
    assert(random_vals[3] == 0);
    ...
    printf("Cocktail shaker sort tests passed\n");
}

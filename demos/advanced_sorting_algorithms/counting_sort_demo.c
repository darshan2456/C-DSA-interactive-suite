#include "advanced_sorting.h"
#include "safe_input.h"
#include "sorting_visualizer.h"
#include "telemetry.h"
#include <stdio.h>
#include <stdlib.h>
#include <time.h>

void counting_sort_demo(void)
{
    int length_of_array;
    while (1)
    {
        printf("\n\nCounting sort demo");
        int status = safe_input_int(&length_of_array,
                                    "\nenter the number of elements in the array "
                                    "(between 1 and 100), enter '-1' to exit:- ",
                                    1, 100);

        if (status == 0)
        {
            continue;
        }

        if (status == INPUT_EXIT_SIGNAL)
        {
            printf("\nExiting counting sort demo....\n");
            return;
        }

        int arr[length_of_array];

        for (int i = 0; i < length_of_array; i++)
        {
        retry:
            printf("\nenter element number %d (between -1000 and 10000), enter '-1' to exit:- ", i);
            int element_status = safe_input_int(&arr[i], NULL, -1000, 10000);

            if (element_status == INPUT_EXIT_SIGNAL)
            {
                printf("\nExiting counting sort demo.....\n");
                return;
            }

            if (element_status == 0)
            {
                goto retry;
            }
        }

        int min_val = arr[0];
        int max_val = arr[0];
        for (int i = 1; i < length_of_array; i++)
        {
            if (arr[i] < min_val)
            {
                min_val = arr[i];
            }
            if (arr[i] > max_val)
            {
                max_val = arr[i];
            }
        }

        long long range_ll = (long long)max_val - min_val + 1;
        if (range_ll > 0 && range_ll <= 100000)
        {
            size_t range = (size_t)range_ll;
            int* count = (int*)calloc(range, sizeof(int));
            if (count != NULL)
            {
                for (int i = 0; i < length_of_array; i++)
                {
                    count[arr[i] - min_val]++;
                }

                printf("\nGenerated Frequency/Count Array:\n");
                printf("Value  : ");
                for (size_t i = 0; i < range; i++)
                {
                    printf("%5d ", min_val + (int)i);
                }
                printf("\nCount  : ");
                for (size_t i = 0; i < range; i++)
                {
                    printf("%5d ", count[i]);
                }
                printf("\n");
                free(count);
            }
        }

        clock_t start_t = clock();
        counting_sort(arr, length_of_array);
        clock_t end_t = clock();
        double total_t = (double)(end_t - start_t) / CLOCKS_PER_SEC;

        printf("\nThe array sorted by counting sort is:- ");
        for (int i = 0; i < length_of_array; i++)
        {
            printf("%d", arr[i]);
            if (i < length_of_array - 1)
            {
                printf(",");
            }
        }
        printf("\nTotal CPU time taken:- %f seconds\n", total_t);
    }
}

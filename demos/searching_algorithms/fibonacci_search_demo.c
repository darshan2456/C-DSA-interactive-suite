...
void fibonacci_search_demo(void)
{
    while (1)
    {
        ...
        int length_of_array;
        int target;
        int fibonacci_search_status;

        printf("\nFibonacci search demo :- \n");
        fibonacci_search_status = safe_input_int(
            &length_of_array, "\nenter length of array, (between 1 and 100), enter '-1' to exit:- ",
            1, 100);

        if (fibonacci_search_status == INPUT_EXIT_SIGNAL)
        {
            printf("\nExiting Fibonacci search demo.\n");
            return;
        }

        if (fibonacci_search_status == 0)
        {
            continue;
        }
        ...
        start_t = clock();
        int res = fibonacci_search(arr, target, length_of_array);
        end_t = clock();
        ...
        printf("\ntotal CPU time taken for Fibonacci search:- %f seconds", total_t);
